...
public class ProductController {
	...
	public void add(@RequestBody Product p, @PathVariable("category_id") int id){
		...
		if (p.getNature().toString().equals("Alimentaire"/**.toLowerCase()*/)){
			...
			Product p1 = new Product(p.getName(),p.getCode(),p.getPrice(),p.getImage(),p.getQuantity(),p.getExpDate(),p.getGeneralrate(),
					...
					,p.getProteines(),p.getSel(),p.getFruits(),p.getMatiere(),p.getDescription());
			...
			Product p1 = new Product (p.getName(),p.getCode(),p.getPrice(),p.getImage(),p.getQuantity(),p.getExpDate(),p.getGeneralrate(),p.getNature(),p.getMatiere(),p.getDescription()); // ajouter tous leess prop sans les energies
			ProductCategory cat = pcr.findById(id).get();
			p1.setProductCategory(cat);
			String category_name =cat.getCategoryName();
			p1.setCategoryname(category_name);
	...
	@GetMapping(value ="/findproductbynature/{nature}")
	@ResponseBody
	public List<Product> findproductbynature(@PathVariable Nature nature){
		List<Product> productresponse =  (List<Product>) productserviceimpl.retrieveByNature(nature);
		return productresponse;
	...
	@GetMapping("/promotion/{id}/{pourcentage}")
	@ResponseBody
	...
	public int Ecoscore(@PathVariable("id") int id ){
	    ...
	    else if ((matiere== "bois")|(matiere== "tissu"))
	    {eco=2;}
